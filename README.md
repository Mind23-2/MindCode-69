...
- [模型描述](#模型描述)
    - [性能](#性能)
        - [评估性能](#评估性能)
            - [ImageNet-1k上的ResNet50+BAM](#ImageNet-1k上的ResNet50+BAM)
        - [推理性能](#推理性能)
            - [ImageNet-1k上的ResNet50+BAM](#ImageNet-1k上的ResNet50+BAM)
...
ResNet50+BAM的作者提出了一个简单但是有效的Attention模型——BAM，它可以结合到任何前向传播卷积神经网络中。作者将BAM放在了ResNet网络中每个stage之间，多层BAMs形成了一个分层的注意力机制，这有点像人类的感知机制，BAM在每个stage之间消除了像背景语义特征这样的低层次特征，然后逐渐聚焦于高级的语义——明确的目标。
...
├── model_zoo
    ...
    ├── ResNet50+BAM
        ...
        │   ├──config.py             // 参数配置
        │   ├──dataset.py            // 创建数据集
        │   ├──ResNet50_BAM.py       // ResNet50+BAM架构
        ├── eval.py                  // 评估脚本
...
- 配置ResNet50+BAM和ImageNet-1k数据集。
  ...
  'keep_checkpoint_max':40 # 最多保存40个ckpt模型文件
  'checkpoint_path':'./ckpt_0/train_resnet50-bam_imagenet-156_10009.ckpt'  # checkpoint文件保存的绝对全路径
  ```
